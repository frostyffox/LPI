# -*- coding: utf-8 -*-
"""covid_assignment.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1Ftq3PJeH2pWjC-bY57v_bDOSslUr0CwF

Summary:
Instead of using a classic EDA approach , I was interested in understanding the missingness of the data, the relationship between deaths , cases and some socio-economic factors and finally the situation regarding hospitalisations.

PLOTS:
- 1-D heat map of missing values per column
- Box plots of cases per million, by continent
- Facet grid of cases per million by continent (not very efficient)
- Bar plot for average CFR by continent
- Regplot for correlation between car and HDI, by continent
- Heat map for correlation between deaths, cases and indicators
- Box plot: hospitalisation burden by continent
- Global hospitalisation trend over time

In terms of analysis I would have loved to
- make a comparison between “developed” and “developing” countries
- Better question the correlation between some factors (age, smoking, life expectancy, etc
"""
...
"""In this case I decided to use a single column heatmap because column that are mostly complete or mostly emply are easily visible.
...
i can guess few countries are contributing with many cases -> right skewed distributiin
"""
...
I use the boxplot because i have categorical (continent) vs numeric (cases) data and within the categorical there are multiple outbreaks (different countries).
"""
...
"""Seems like the highes cfr were in south america and africa, even if North america and Europe also had countries with many deaths.
The barplot is more appropriate in this case because i can distinguish the continents (qualitative) and clearly see the different height of the bars (quantitative).
...
"""I see a ~0.754 correlation between HDI and GDP so i feel confident that HDI is quite reliable to understand countries wealth"""
...
"""I think this plot was the right choice because we are dealing with 2 quantitative variables and i have to map the average correlation for each country. The regression line represents the best model to explain the relationship between HDI and CFR (average) by continent.
...
"""I see the most important peaks in the winter of 2020-2021 (as expected), winter of 2021-2022 (maybe the coincidence with winter made people more vulneranle)."""